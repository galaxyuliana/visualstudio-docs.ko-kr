...
- [테스트 탐색기를 사용하여 네이티브 코드 유닛 테스트](https://msdn.microsoft.com/8a09d6d8-3613-49d8-9ffe-11375ac4736c)  
...
### <a name="BKMK_Running_and_viewing_tests_from_the_Test_Explorer_toolbar"></a> 테스트 실행 및 보기  
 ...
 사용자는 **모두 실행** 을 선택해서 모든 테스트를 실행하거나 **실행** 을 선택해서 실행할 테스트 하위 집합을 선택할 수 있습니다. 일련의 테스트들을 실행한 다음에는 테스트 탐색기 창의 맨 아래에 테스트 실행의 요약이 나타납니다. 테스트를 선택하면 아래쪽 창에 해당 테스트의 세부 정보가 표시됩니다. **테스트 열기**를 상황에 맞는 메뉴에서(키보드: F12) 선택된 된 테스트에 대 한 소스 코드를 표시 합니다.  
 ...
 **A:** 테스트 탐색기를 사용 하 여 테스트에 대 한 디버깅 세션을 시작 합니다. Visual Studio 디버거에서 코드를 단계별로 실행하면 단위 테스트 및 테스트 중인 프로젝트 간을 앞뒤로 매끄럽게 이동할 수 있습니다. 디버깅을 시작하려면  
...
2. 테스트 탐색기에서 테스트 메서드를 선택한 다음 바로 가기 메뉴에서 **선택한 테스트 디버그** 를 선택합니다.  
   ...
   **Q: TDD를 사용 하는 경우 어떻게 코드를 생성 하려면 테스트에서?**  
 ...
 **Q: 외부 종속성이 있는 코드에서 메서드를 테스트할 수는 방법**  
