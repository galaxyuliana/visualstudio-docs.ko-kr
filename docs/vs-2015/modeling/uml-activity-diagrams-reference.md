...
|-|-|-|  
|**셰이프**|**요소**|**설명 및 주요 속성**|  
|1|**동작**|사용자 또는 소프트웨어가 일부 작업을 수행하는 동작의 한 단계입니다.<br /><br /> 토큰이 들어오는 모든 흐름에 도착하면 작업을 시작할 수 있습니다. 작업이 끝나면 보내는 모든 흐름에서 토큰이 전송됩니다.<br /><br /> -   **본문** -세부 정보에는 동작을 지정 합니다.<br />-   **언어** -본문에 있는 식의 언어입니다.<br />-   **로컬 사후 조건** -실행 종료 시 충족 되어야 하는 제약 조건입니다. 작업을 통해 달성되는 목표입니다.<br />-   **로컬 사전 조건** -실행 시작 전에 충족 해야 하는 제약 조건입니다.|  
...
|9|**동작 호출 작업**|다른 동작 다이어그램에서 자세히 정의된 작업입니다.<br /><br /> -   **동기** -활동 종료 될 때까지 작업이 대기 하는 true 인 경우.<br />-   **동작** -호출 되는 동작입니다.|  
