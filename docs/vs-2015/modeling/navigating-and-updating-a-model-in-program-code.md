...
## <a name="in-this-topic"></a>이 항목의 내용
 ...
 도형, 연결선 및 다이어그램과 모델 요소에 대 한 관계는 별도의 항목에서 설명 합니다. 자세한 내용은 [방법: 다이어그램](../misc/how-to-navigate-and-update-a-diagram.md)을 탐색 하 고 업데이트 합니다.
...
- `PropagatesDelete` 플래그가 true 인 역할의 요소입니다. 예를 들어 요소를 표시 하는 셰이프는 삭제 됩니다.
  ...
  기본적으로 모든 포함 관계 `PropagatesDelete` 는 대상 역할에 적용 됩니다. 을 삭제 `familyTree`해도는삭제 되지 않지만 `familyTree.Delete()` 모든를 `Persons`삭제 합니다. `henry` 자세한 내용은 [삭제 동작 사용자 지정](../modeling/customizing-deletion-behavior.md)을 참조 하세요.
...
### <a name="presentation-elements"></a>프레젠테이션 요소
 ![기본 도형 및 요소 형식의 클래스 다이어그램](../modeling/media/dslshapesandelements.png "DSLshapesAndElements")
...
|다이어그램|<xref:Microsoft.VisualStudio.Modeling.Diagrams.Diagram>|
 ...
 셰이프는 두 집합에 자식 셰이프를 포함할 수 있습니다. `NestedChildShapes` 집합의 도형은 부모의 경계 상자로 한정 됩니다. `RelativeChildShapes` 목록에 있는 셰이프는 부모 경계 외부 또는 부분적으로 나타날 수 있습니다 (예: 레이블 또는 포트). 다이어그램에는 `Parent`및 `RelativeChildShapes` 이 없습니다.
...
## <a name="see-also"></a>관련 항목
 <xref:Microsoft.VisualStudio.Modeling.ModelElement>[도메인 특정 언어의 유효성 검사](../modeling/validation-in-a-domain-specific-language.md) [도메인별 언어에서 코드 생성](../modeling/generating-code-from-a-domain-specific-language.md) [방법: 트랜잭션을 사용 하 여 모델](../modeling/how-to-use-transactions-to-update-the-model.md) 을 업데이트 합니다. [Visual Studio를 사용 하 여 모델 통합 Modelbus](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [변경 내용에 대 한 응답 및 전파](../modeling/responding-to-and-propagating-changes.md)
