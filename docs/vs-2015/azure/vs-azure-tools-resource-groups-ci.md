...
8. **스크립트 인수** 상자에 다음 매개 변수를 입력합니다(한 줄로). Visual Studio에서 스크립트를 실행하면 **출력** 창에서 VS가 매개 변수를 어떻게 사용하는지 확인할 수 있습니다. 이 정보를 빌드 단계에서 매개 변수 값을 설정하기 위한 출발점으로 사용할 수 있습니다.
   ...
   | -StorageAccountName |이 배포에 대한 스테이지 아티팩트에 사용되는 스토리지 계정 이름. 이 매개 변수는 배포에 대한 아티팩트를 준비하는 경우에만 사용됩니다. 이 매개 변수가 제공되는 경우 이전 배포 동안 스크립트에서 스토리지 계정을 만들지 않은 경우 새 스토리지 계정이 만들어집니다. 매개 변수를 지정하는 경우 스토리지 계정이 이미 있어야 합니다. |
   | -StorageAccountResourceGroupName |스토리지 계정과 연결된 리소스 그룹의 이름. 이 매개 변수는 StorageAccountName 매개 변수에 대한 값을 제공하는 경우에만 필요합니다. |
...
5. **Azure 파일 복사** 작업을 선택하고 해당 값을 입력합니다.
   ...
   * Azure RM 구독 - **Azure 구독** 드롭다운 목록 상자에서 사용하려는 스토리지 계정에 대한 구독을 선택합니다. 구독이 나타나지 않으면 **관리** 링크 옆에 있는 **새로 고침** 단추를 선택합니다.
   * 대상 형식 - **Azure Blob** 선택
   ...
   * 스토리지 컨테이너 SAS 토큰 - **artifactsLocationSasToken** 입력
