...
|**응용 프로그램 포괄 시간(응용 프로그램 시간)**|애플리케이션 시간에는 컨텍스트 전환, 입/출력 작업 등 운영 체제에 대한 호출에 소요된 시간이 포함되지 않습니다.<br /><br /> - **호출 함수**의 경우 함수가 호출한 선택된 함수의 인스턴스를 실행하는 데 소요된 애플리케이션 시간의 크기입니다. 선택된 함수가 호출한 함수에서 소요된 시간은 포함됩니다.<br />- **함수 본문**의 경우 선택된 함수의 코드를 실행하는 데 경과된 총 애플리케이션 시간의 크기입니다. 호출된 함수에서 소요된 시간은 포함되지 않습니다.<br />- 호출된 함수의 경우 선택된 함수가 호출한 함수의 인스턴스를 실행하는 데 소요된 애플리케이션 시간의 크기입니다. 총 시간에 해당 함수가 호출한 함수에서 소요된 시간도 포함됩니다.|  
