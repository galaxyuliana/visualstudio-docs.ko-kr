...
|기능|설명|F#에서 지원되는지 여부|
|-------|-----------|----------------|
|자동 주석|코드의 섹션을 주석으로 처리하거나 주석 처리를 제거할 수 있습니다.|적용|
|서식 자동 지정|표준 들여쓰기 및 스타일을 사용하여 코드의 서식을 다시 지정합니다.|아니요|
|책갈피|편집기에서 위치를 저장할 수 있습니다.|적용|
|들여쓰기 변경|선택한 줄을 들여쓰거나 내어씁니다.|적용|
|스마트 들여쓰기|F# 범위 지정 규칙에 따라 커서를 자동으로 들여쓰고 들여쓰기를 취소합니다.|적용|
|[텍스트 찾기 및 바꾸기](finding-and-replacing-text.md)|파일, 프로젝트 또는 솔루션에서 검색하고 잠재적으로 텍스트를 변경할 수 있습니다.|적용|
|.NET Framework API에 대한 정의로 이동|.NET Framework API에 커서를 놓으면 .NET Framework 메타데이터에서 생성된 코드를 보여 줍니다.|아니요|
|사용자 정의 API에 대한 정의로 이동|지정한 프로그램 엔터티에 커서를 놓으면 엔터티가 정의된 위치의 코드에서 해당 위치로 커서를 이동합니다.|적용|
|줄 이동|줄 번호에 따라 파일에서 특정 줄로 이동할 수 있습니다.|적용|
|파일의 위쪽에 있는 탐색 모음|함수 이름과 같은 코드에 있는 위치로 점프할 수 있습니다.|적용|
|블록 구조 가이드|미리 보기에 대해 가리킬 수 있는 F# 범위를 나타내는 지침을 보여 줍니다.|적용|
|[개요](outlining.md)|더 간단한 보기를 만드는 코드의 섹션을 축소할 수 있습니다.|적용|
|탭으로 바꿈|공백을 탭으로 변환합니다.|적용|
|유형 색 지정|정의된 형식 이름을 특별한 색으로 표시합니다.|적용|
|빠른 찾기. 빠른 찾기, 찾기 및 바꾸기 창을 참조하세요.|파일 또는 프로젝트에서 검색할 수 있습니다.|적용|
|**Ctrl**+**키를 누른 채로** 정의로 이동을 클릭|**Ctrl** 키를 누른 채로 F# 기호를 클릭하면 정의로 이동을 호출할 수 있습니다.|적용|
|QuickInfo에서 정의로 이동|도구 설명 내에서 클릭할 수 있는 기호로, 정의로 이동을 호출합니다.|적용|
|전체로 이동|**Ctrl**+**T**를 통해 모든 F# 구문에 대한 전역, 유사 항목 일치를 탐색할 수 있습니다.|적용|
|인라인 이름 바꾸기|기호 인라인의 모든 항목의 이름을 바꿉니다.|적용|
|모든 참조 찾기|코드 베이스에서 기호의 모든 발생을 찾습니다.|적용|
|이름 코드 수정 사항 간소화|F# 기호에 대한 불필요한 한정자를 제거합니다.|적용|
|사용하지 않는 `open` 문 코드 수정 제거|문서에서 모든 불필요한 `open` 문을 제거합니다.|적용|
|사용하지 않는 값 코드 수정|사용하지 않는 식별자의 이름을 바꾸는 것을 밑줄로 제안합니다.|적용|
...
|기능|설명|F#에서 지원되는지 여부|
|-------|-----------|----------------|
|인터페이스 자동 구현|인터페이스 메서드에 대한 코드 스텁을 생성합니다.|적용|
|코드 조각|일반적인 코딩 구문의 라이브러리에서 항목으로 코드를 삽입합니다.|아니요|
|단어 자동 완성|입력할 때 단어와 이름을 완성시켜 입력을 저장합니다.|적용|
|자동 완성|사용하도록 설정하면 사용자가 입력할 때 단어 자동 완성 기능이 사용자가 하나를 선택하거나 **Ctrl**+**스페이스바**를 누를 때까지 기다리지 않고 첫 번째 일치 항목을 선택합니다.|적용|
|열려 있지 않은 네임스페이스의 기호에 대한 완성 제공|자동 완성 기능을 사용하면 열려 있지 않은 네임스페이스에 상주하는 일치하는 기호가 제안되어 선택하는 경우 해당하는 `open` 문으로 완성됩니다.|적용|
|코드 요소 생성|다양한 구문에 대한 스텁 코드를 생성할 수 있습니다.|아니요|
|멤버 목록|멤버 액세스 연산자(.)을 입력하면 형식의 멤버가 표시됩니다.|적용|
|Usings/Open 구성|C#의 **using** 문 또는 F#의 **open** 지시문에 의해 참조되는 네임스페이스를 구성합니다.|아니요|
|매개 변수 정보|함수 호출을 입력할 때 매개 변수에 대한 유용한 정보를 표시합니다.|적용|
|요약 정보|코드의 식별자에 대한 전체 선언을 표시합니다.|적용|
|자동 중괄호 완료|트랜잭션 방식으로 자동으로 F# 중괄호 유사 구문을 생성을 완료합니다.|적용|
...
|중단점|디버깅하는 동안 특정 지점에서 코드 실행을 일시 중지할 수 있습니다.|적용|
|조건부 중단점|실행을 일시 중지할지 여부를 결정하는 조건을 테스트하는 중단점을 설정합니다.|적용|
...
|기록 디버깅|이전에 실행된 코드를 시작할 수 있습니다.|적용|
|지역 창|로컬로 정의된 값 및 변수를 보여 줍니다.|적용|
|커서까지 실행|커서를 포함하는 줄에 도달할 때까지 코드를 실행할 수 있습니다.|적용|
|한 단계씩 코드 실행|실행을 발전시키고 함수 호출으로 이동할 수 있습니다.|적용|
|프로시저 단위 실행|현재 스택 프레임에서 실행하면서 지난 함수 호출로 이동할 수 있습니다.|적용|
...
|[오류 목록 창](reference/error-list-window.md)|코드의 오류 목록을 보여 줍니다.|적용|
|[F# Interactive](/dotnet/fsharp/tutorials/fsharp-interactive/)|F# 코드를 입력(또는 복사 및 붙여넣기)하고, 즉시 실행하여 프로젝트를 독립적으로 빌드할 수 있습니다. F# 대화형 창은 REPL(Read, Evaluate, Print Loop)입니다.|적용|
...
|[출력 창](reference/output-window.md)|빌드 출력을 표시합니다.|적용|
|성능 분석|코드의 성능을 측정하기 위한 도구를 제공합니다.|적용|
|속성 창|포커스가 있는 개발 환경에서 개체의 속성 편집을 표시하고 사용하도록 설정합니다.|적용|
|서버 탐색기|다양한 서버 리소스와 상호 작용하는 방법을 제공합니다.|적용|
|솔루션 탐색기|프로젝트 및 파일을 보고 관리할 수 있습니다.|적용|
|작업 목록|코드와 관련된 작업 항목을 관리할 수 있습니다.|아니요|
|테스트 프로젝트|코드를 테스트하는 데 도움이 되는 기능을 제공합니다.|아니요|
|도구 상자|텍스트 또는 코드의 섹션 및 컨트롤과 같이 끌기 가능한 개체를 포함하는 탭을 표시합니다.|적용|
