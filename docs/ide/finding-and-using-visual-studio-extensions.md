...
**확장 및 업데이트** 대화 상자를 사용하여 Visual Studio 확장을 설치하고 관리합니다. **확장 및 업데이트** 대화 상자를 열려면 **도구** > **확장 및 업데이트**를 선택하거나 **빠른 실행** 창에서 **확장**을 입력합니다.

![Visual Studio의 확장 및 업데이트 대화 상자](media/finding-using-visual-studio-extensions/extensions-and-updates.png)
...
오류 또는 악의적인 코드를 포함할 수 있는 확장으로부터 시스템을 보호하기 위해, Visual Studio가 일반 사용자 권한으로 실행되는 경우에만 로드되도록 사용자별 확장을 제한할 수 있습니다. 이는 Visual Studio가 관리자 권한으로 실행되는 경우에는 사용자별 확장을 사용하지 않도록 설정됨을 의미합니다. 이렇게 하려면 **확장 및 업데이트** 옵션 페이지로 이동합니다(**도구 > 옵션** > **환경** > **확장 및 업데이트**). **관리자로 실행할 때 사용자 확장별 로드** 확인란을 선택 취소한 다음 Visual Studio를 다시 시작합니다.

## <a name="automatic-extension-updates"></a>자동 확장 업데이트

Visual Studio Marketplace에서 새 버전을 사용할 수 있으면 확장이 자동으로 업데이트됩니다. 새 버전의 확장이 검색되어 백그라운드에 설치됩니다. 다음에 Visual Studio를 시작하며 새 버전의 확장이 실행됩니다.
