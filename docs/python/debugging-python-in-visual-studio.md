...
HTML, XML 및 JSON 시각화는 구문 강조 표시 및 트리 뷰가 있는 별도의 팝업 창에 표시됩니다.
...
| 명령 | 인수 | 설명 |
| --- | --- | --- |
| `$continue`에서 `$cont`에서 `$c` | 현재 문에서 프로그램 실행을 시작합니다. |
...
| `$stepin`에서 `$step`에서 `$s` | 가능한 경우 다음 함수 호출을 단계별로 실행합니다. |
| `$stepout`에서 `$return`에서 `$r` | 현재 함수에서 나갑니다. |
| `$stepover`에서 `$until`에서 `$unt` | 다음 함수 호출을 건너뜁니다. |
...
| `$where`에서 `$w`에서 `$bt` | 현재 스레드의 프레임을 나열합니다. |
