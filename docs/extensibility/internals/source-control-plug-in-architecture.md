...
## <a name="source-control-plug-in-resources"></a>소스 제어 플러그 인 리소스
 원본 제어 플러그 인 만들기 및 버전 관리 응용 프로그램을 연결 하는 데 리소스를 제공 합니다 [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] IDE. 원본 제어 플러그 인에 통합 될 수 있도록 소스 제어 플러그 인에서 구현 해야 하는 API 사양을 포함 된 [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] IDE. 코드 샘플도 포함 (작성 된 C++)를 기본 원본 제어 플러그 인 설명 함수의 구현 형태가 필수 원본 제어 플러그 인 API를 사용 하 여 준수를 구현 하는 합니다.
