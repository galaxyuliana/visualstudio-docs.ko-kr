...
#### <a name="use-win32-and-wpf-versions-of-ivstextview"></a>Win32와 WPF 버전 IVsTextView의 사용
 ...
 호출 텍스트 뷰 WPF 모드로 설정 해야 합니다 <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView.Initialize%2A> 전달 [TextViewInitFlags3.VIF_NO_HWND_SUPPORT](<xref:Microsoft.VisualStudio.TextManager.Interop.TextViewInitFlags3.VIF_NO_HWND_SUPPORT>) 으로 초기화 중 하나에서 플래그를 `InitView` 매개 변수입니다. 가져올 수 있습니다 합니다 <xref:System.Windows.FrameworkElement> 를 호출 하 여 <xref:Microsoft.VisualStudio.Shell.Interop.IVsUIElementPane.CreateUIElementPane%2A>입니다.
