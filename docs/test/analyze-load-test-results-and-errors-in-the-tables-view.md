...
|열|설명|기본적으로 표시되는지 여부|
|-|-|-|
|**요청**|요청의 URL입니다. 예를 들어 *home.html* 또는 *orange-arrow.gif*입니다.|적용|
|**시나리오**|시나리오 이름입니다.|적용|
|**테스트**|테스트의 이름입니다.|적용|
|**합계**|부하 테스트 실행 도중 해당 웹 성능 테스트 요청이 생성된 총수입니다. 총수에는 성공한 요청과 실패한 요청이 포함되지만 캐시된 요청은 웹 서버로 전송되지 않으므로 포함되지 않습니다.|적용|
|**전달**|요청이 생성되어 성공한 횟수입니다.|아니요|
|**실패**|요청이 생성되어 실패한 횟수입니다. 이 열의 항목은 하이퍼링크로 나타납니다. **부하 테스트 오류** 대화 상자에서 개별 오류 목록을 보기 위해 하이퍼링크를 선택할 수 있습니다. 자세한 내용은 [부하 테스트 결과 분석](../test/analyze-load-test-results-using-the-load-test-analyzer.md)을 참조하세요.|적용|
...
|**응답 시간**|요청이 웹 서버로 전송된 시점에서 요청에 대한 전체 응답이 수신된 시점 사이의 평균 시간입니다. 단위는 초입니다.|적용|
|**콘텐츠 길이**|요청에 대한 응답의 평균 콘텐츠 길이입니다. 단위는 바이트입니다.|적용|
...
|**테스트**|테스트의 이름입니다.|적용|
|**시나리오**|시나리오 이름입니다.|적용|
|**합계**|시나리오에서 테스트가 실행된 총 횟수입니다. 여기에는 테스트가 성공한 횟수와 실패한 횟수가 포함됩니다.|적용|
|**전달**|시나리오에서 테스트가 실행되어 성공한 횟수입니다.|적용|
|**실패**|시나리오에서 테스트가 실행되었지만 실패한 횟수입니다. 이 열의 항목은 하이퍼링크로 나타납니다. **부하 테스트 오류** 대화 상자에서 개별 오류 목록을 보기 위해 하이퍼링크를 선택할 수 있습니다. 자세한 내용은 [부하 테스트 결과 분석](../test/analyze-load-test-results-using-the-load-test-analyzer.md)을 참조하세요.|적용|
|**테스트/초**|부하 테스트 실행 도중 초당 테스트 비율입니다.|적용|
|**전달/초**|부하 테스트 실행 도중 해당 테스트의 인스턴스가 성공한 초당 비율입니다.|아니요|
|**실패/초**|부하 테스트 실행 도중 해당 테스트의 인스턴스가 실패한 초당 비율입니다.|아니요|
|**테스트 시간**|부하 테스트 실행 도중 테스트를 실행하는 데 걸린 평균 시간입니다. 단위는 초입니다.|적용|
|**90% 테스트 시간**|테스트 시간의 90번째 백분위수 값입니다.|아니요|
|**95% 테스트 시간**|테스트 시간의 95번째 백분위수 값입니다.|적용|
...
|**시나리오**|시나리오 이름입니다.|적용|
|**테스트**|테스트의 이름입니다.|적용|
|**합계**|부하 테스트 도중 생성된 총 트랜잭션 수입니다.|적용|
...
|열|설명|기본적으로 표시되는지 여부|
|-|-|-|
|형식|오류의 유형입니다. 예를 들어 HttpError가 표시됩니다.|적용|
|하위 형식|오류의 하위 형식입니다. 예를 들어 LoadTestException이 표시됩니다.|적용|
|개수|부하 테스트 도중 해당 유형의 오류가 발생한 개수입니다. 이 열의 항목은 하이퍼링크로 나타납니다. 하이퍼링크를 선택하여 개별 오류 목록을 확인할 수 있습니다.|적용|
|마지막 메시지|오류를 설명하는 메시지입니다. 예를 들어 404 - NotFound가 표시됩니다.|적용|
