...
|열|설명|기본적으로 표시되는지 여부|
|-|-|-|
|시간|부하 테스트 도중 위반이 발생한 시간입니다.|적용|
|컴퓨터|테스트 도중 위반이 발생한 컴퓨터의 이름입니다. **참고:**  Rig에서 부하 테스트를 실행하는 경우 이 정보가 중요합니다.|적용|
|범주|위반이 발생한 성능 카운터 범주입니다.|적용|
|카운터|위반이 발생한 성능 카운터 이름입니다.|적용|
|인스턴스|위반이 발생한 성능 카운터 인스턴스입니다.|적용|
|메시지|임계값 위반을 설명하는 메시지입니다. 예: **값 5가 중요 임계값 0을 초과합니다.**|적용|
