...
C# 코드, 중단점 및 현재 실행 줄은 자동으로 강조 합니다. 에 대 한 C++ 코드를 켤 수 있습니다 선택 하 여 중단점 및 현재 줄의 강조 표시 **Tools** (또는 **디버그**) > **옵션**  >  **디버깅** >  **중단점과 현재 문의 전체 소스 줄 강조 표시 (C++ 만)** .
...
### <a name="set-a-function-breakpoint-using-a-memory-address-native-c-only"></a>메모리 주소를 사용 하 여 함수 중단점 설정 (네이티브 C++ 만)
...
2. 에 **자동**를 **조사식**, 또는 **지역** 창에서 속성을 마우스 오른쪽 단추로 클릭 하 고 선택 **값이 변경 될 때 중단** 상황에 맞는 메뉴입니다.
...
- 여기서는 조사식 4 바이트 32 비트 하드웨어에 64 비트 하드웨어에 8 바이트 보다 큽니다. 이 x86 제한 아키텍처입니다.
